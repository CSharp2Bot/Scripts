	...
	private static double xp() {
		for (TreeTypes tree : TreeTypes.values()) {
			if (Vars.progressiveLevel) {
				if (ProgLevelSetup.getTree().equals(ProgLevelSetup.Tree.NORMAL)) {
					Vars.treeType = TreeTypes.NORMAL.getName();
					return TreeTypes.NORMAL.getXp();
				}
				if (ProgLevelSetup.getTree().equals(ProgLevelSetup.Tree.OAK)) {
					Vars.treeType = TreeTypes.OAK.getName();
					return TreeTypes.OAK.getXp();
				}
				if (ProgLevelSetup.getTree().equals(ProgLevelSetup.Tree.WILLOW)) {
					Vars.treeType = TreeTypes.WILLOW.getName();
					return TreeTypes.WILLOW.getXp();
				}
			} else {
